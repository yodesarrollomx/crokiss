/* =============================================================
   ...
   Genera una lámina profesional como un solo SVG en milímetros
   de papel: marco, plano a escala, cotas, norte, escala gráfica,
   cajetín Aurum, cuadro de datos, leyenda de muebles y fachada
   (si plan-elev.js está cargado). Vista previa en overlay e
   impresión/PDF con window.print(). JS puro, sin dependencias.
   API:  PlanSheet.open(editor)   ← botón "⎙ Lámina / PDF"
   Punto de extensión fachada:    PlanSheet.setElevProvider(fn)
     fn(geom) → { svg, widthM, heightM, label }  (grupo SVG en px,
     mismo PPM del plano; null si no se puede generar)
   ============================================================= */
(function () {
  'use strict';
  ...
  /* ---------- construcción de la lámina completa ---------- */
  function buildSheet(geom, opts) {
    var F = FORMATOS[opts.formato] || FORMATOS.carta;
    var W = F.w, H = F.h, M = 10;                    // margen exterior
    var CAJ = 62;                                    // ancho del cajetín (columna derecha)
    ...
    var quiereElev = elevProvider && opts.conFachada !== false;
    var elev = quiereElev ? elevProvider(geom) : null;

    // reparto vertical: plano arriba, fachada abajo (si existe)
    var planoH = elev ? dz.h * 0.62 : dz.h;
    var holg = 14;                                   // espacio para cotas alrededor del plano
    // escala: la menor estándar en la que cabe el plano
    var availW = dz.w - holg * 2, availH = planoH - holg * 2;
    var escala = 0;
    for (var i = 0; i < ESCALAS.length; i++) {
      var e = ESCALAS[i];
      if ((maxX * 1000) / e <= availW && (maxY * 1000) / e <= availH) { escala = e; break; }
    }
    if (!escala) {     // terreno enorme para este formato: escala exacta redondeada a 25 (nunca desbordar el marco)
      escala = Math.ceil(Math.max((maxX * 1000) / availW, (maxY * 1000) / availH) / 25) * 25;
    }
    var mmPerM = 1000 / escala, pxToMm = mmPerM / PPM;

    var s = '';
    // marco doble
    s += el('rect', { x: M, y: M, width: W - M * 2, height: H - M * 2, fill: 'none', stroke: TINTA, 'stroke-width': 0.5 });
    s += el('rect', { x: M + 2, y: M + 2, width: W - M * 2 - 4, height: H - M * 2 - 4, fill: 'none', stroke: TINTA, 'stroke-width': 0.2 });

    // ----- plano centrado en su zona -----
    var pw = maxX * mmPerM, ph = maxY * mmPerM;
    ...
    s += cota(px0, py0 - 5, px0 + pw, py0 - 5, fmt1(maxX) + ' m');
    s += cota(px0 - 5, py0, px0 - 5, py0 + ph, fmt1(maxY) + ' m');
    ...
    // escala gráfica (referencia robusta aunque impriman "ajustar a página")
    var segM = escala <= 75 ? 1 : escala <= 150 ? 2 : 5;     // metros por segmento
    var segMM = segM * mmPerM, gx = dz.x + 2, gy = dz.y + planoH - 4;
    for (var k = 0; k < 4; k++) {
      s += el('rect', { x: gx + k * segMM, y: gy, width: segMM, height: 1.6, fill: k % 2 ? '#fff' : TINTA, stroke: TINTA, 'stroke-width': 0.2 });
      s += txt(gx + k * segMM, gy + 4.4, String(k * segM), 2.6, { ancla: 'middle', fill: MUTED });
    }
    s += txt(gx + 4 * segMM, gy + 4.4, (4 * segM) + ' m', 2.6, { ancla: 'middle', fill: MUTED });

    // ----- fachada (punto de extensión de plan-elev.js) -----
    if (elev) {
      var fy0 = dz.y + planoH + 4;
      var fAvailH = dz.h - planoH - 8, fAvailW = dz.w - holg;
      var fEsc = 0;                                   // la fachada usa SU mejor escala estándar (se ve más grande)
      for (var j = 0; j < ESCALAS.length; j++) {
        var e2 = ESCALAS[j];
        if (elev.widthM * 1000 / e2 <= fAvailW && elev.heightM * 1000 / e2 <= fAvailH - 8) { fEsc = e2; break; }
      }
      if (!fEsc) fEsc = Math.ceil(Math.max(elev.widthM * 1000 / fAvailW, elev.heightM * 1000 / (fAvailH - 8)) / 25) * 25;
      var fScale = (1000 / fEsc) / PPM;
      var fw = elev.widthM * (1000 / fEsc), fh = elev.heightM * (1000 / fEsc);
      var fx0 = dz.x + (dz.w - fw) / 2, fyy = fy0 + (fAvailH - fh - 6) / 2;
      s += el('line', { x1: dz.x, y1: fy0 - 2, x2: dz.x + dz.w, y2: fy0 - 2, stroke: LINEA, 'stroke-width': 0.3 });
      s += el('g', { transform: 'translate(' + fx0 + ' ' + fyy + ') scale(' + fScale + ')' }, elev.svg);
      s += txt(dz.x + 2, fy0 + fAvailH - 1, (elev.label || 'FACHADA PRINCIPAL') + ' · ESC 1:' + fEsc, 4, { peso: 700, esp: 0.8 });
    }

    // ----- cajetín (columna derecha) -----
    var cx0 = W - M - 2 - CAJ, cy0 = M + 2, ch = H - M * 2 - 4;
    s += el('line', { x1: cx0, y1: cy0, x2: cx0, y2: cy0 + ch, stroke: TINTA, 'stroke-width': 0.4 });
    var cx = cx0 + 4, cw = CAJ - 8, yy = cy0 + 10;

    s += txt(cx, yy, 'Cro·Kiss', 8, { peso: 700, esp: 0.6 }); yy += 4.6;
    s += txt(cx, yy, 'EL PRIMER BESO CON TU PROYECTO', 2.4, { fill: MUTED, esp: 0.7 }); yy += 7;
    s += el('line', { x1: cx, y1: yy, x2: cx + cw, y2: yy, stroke: ACENTO, 'stroke-width': 0.7 }); yy += 7;

    function campo(label, valor) {
      valor = String(valor || '—');
      if (valor.length > 26) valor = valor.slice(0, 25) + '…';   // el cajetín nunca se desborda
      s += txt(cx, yy, label.toUpperCase(), 2.5, { fill: MUTED, esp: 0.8 }); yy += 4;
      s += txt(cx, yy, valor, 4.1, { peso: 600 }); yy += 7;
    }
    campo('Proyecto', opts.planName);
    campo('Cliente', opts.cliente);
    campo('Fecha', opts.fecha);
    campo('Terreno', geom.lot ? (geom.lot.w + ' × ' + geom.lot.d + ' m') : (fmt1(maxX) + ' × ' + fmt1(maxY) + ' m'));
    campo('Escala', '1:' + escala + ' (' + F.nombre + ')');

    // datos del plano
    yy += 1;
    s += el('line', { x1: cx, y1: yy, x2: cx + cw, y2: yy, stroke: LINEA, 'stroke-width': 0.3 }); yy += 5.5;
    s += txt(cx, yy, 'DATOS', 2.5, { fill: MUTED, esp: 0.8 }); yy += 4.5;
    var ml = 0; geom.walls.forEach(function (w) { ml += Math.hypot(w.x2 - w.x1, w.y2 - w.y1); });
    var ev2 = geom.elev || {};
    [
      'Huella: ' + fmt1(maxX) + ' × ' + fmt1(maxY) + ' m  (' + fmt1(maxX * maxY) + ' m²)',
    ...
    ].forEach(function (linea) { s += txt(cx, yy, linea, 3.1); yy += 4.4; });
    ...
    if (tipos.length) {
      yy += 2;
      s += el('line', { x1: cx, y1: yy, x2: cx + cw, y2: yy, stroke: LINEA, 'stroke-width': 0.3 }); yy += 5.5;
      s += txt(cx, yy, 'MOBILIARIO', 2.5, { fill: MUTED, esp: 0.8 }); yy += 4.5;
      tipos.slice(0, 10).forEach(function (t) {
        var cat = (FL && FL.CATALOG[t]) || {};
        s += txt(cx, yy, (counts[t] > 1 ? counts[t] + ' × ' : '') + (cat.label || t), 3.1); yy += 4.2;
      });
      if (tipos.length > 10) { s += txt(cx, yy, '… y ' + (tipos.length - 10) + ' más', 3, { fill: MUTED }); yy += 4.2; }
    }

    // pie del cajetín: firma + disclaimer
    var by = cy0 + ch - 16;
    s += el('line', { x1: cx, y1: by, x2: cx + cw, y2: by, stroke: ACENTO, 'stroke-width': 0.7 });
    s += txt(cx, by + 5, 'CroKiss · Aurum Arquitectos', 3.6, { peso: 700 });
    s += txt(cx, by + 9.2, 'aurumarquitectos.com', 2.7, { fill: MUTED });
    s += txt(cx, by + 13.4, 'Anteproyecto ilustrativo — no apto para construcción', 2.3, { fill: MUTED });
  ...
  function ensureStyle() {
    ...
    st.textContent =
      ...
      '#ck_lamina .lam-view{flex:1;overflow:auto;display:flex;align-items:flex-start;justify-content:center;padding:22px;}' +
      '#ck_lamina .lam-view svg{max-width:100%;height:auto;box-shadow:0 12px 40px rgba(0,0,0,.45);}' +
      '@media print{' +
        ...
        'body.ck-lamina-open #ck_lamina .lam-view svg{box-shadow:none;max-width:none;width:100%;height:auto;}' +
      '}';
  ...
  function open(editor) {
    ...
    function rerender() {
      ...
      ov.innerHTML =
        '<div class="lam-bar">' +
          '<b>⎙ Lámina</b><span style="opacity:.65">vista previa del entregable</span><span class="sp"></span>' +
        ...
        '</div>' +
        '<div class="lam-view">' + buildSheet(geom, opts) + '</div>';
  ...
  window.PlanSheet = {
    ...
    _buildSheet: buildSheet                                 // interno: pruebas sin DOM
  };
})();
