...
| `plan-sheet.js` | Lámina de 2 páginas: pág. 1 planta (toda la zona, mejor escala), pág. 2 rejilla 2×2 de fachadas a la misma escala; cajetín reutilizable con "LÁMINA n DE m"; overlay apilado + `window.print()` con page-break por hoja. API: `open`, `_buildPages` (pruebas), `_buildSheet` (alias pág. 1), `setElevProvider` (fallback) |
| `plan-elev.js` | Fachadas F4: `facade(geom, dir)` con dir S/N/E/O (espejado: S s=x · N s=maxX−x · E s=maxY−y · O s=y), `allFacades(geom)`, `frontFacade` alias; oclusión por barrido 1D, vanos con `data-kind`/`data-id`, líneas de nivel NPT, pretil según `geom.elev.cubierta`/`pretil` |
