...
(function () {
  ...
  function PlanEditor(mountId) {
    ...
    function normalize() {
      ...
      geom.doors = geom.doors || [];
      geom.sliders = geom.sliders || [];
      geom.furniture = geom.furniture || [];
    ...
    function render() {
      ...
      geom.walls.forEach((w) => {
        ...
        if (isSel) {
          [['p1', w.x1, w.y1], ['p2', w.x2, w.y2]].forEach(([p, mx, my]) => {
            hit += el('circle', { cx: X(mx), cy: Y(my), r: 7, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'wall', 'data-id': w.id, 'data-part': p, style: 'cursor:crosshair' });
          });
        }
      });
      ...
      geom.windows.forEach((wn) => {
        ...
        if (wn.wall === 'h') {
          ...
          if (isSel) { hit += el('circle', { cx: X(wn.a), cy: Y(wn.fixed), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'window', 'data-id': wn.id, 'data-part': 'a', style: 'cursor:ew-resize' }); hit += el('circle', { cx: X(wn.b), cy: Y(wn.fixed), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'window', 'data-id': wn.id, 'data-part': 'b', style: 'cursor:ew-resize' }); }
        } else {
          ...
          if (isSel) { hit += el('circle', { cx: X(wn.fixed), cy: Y(wn.a), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'window', 'data-id': wn.id, 'data-part': 'a', style: 'cursor:ns-resize' }); hit += el('circle', { cx: X(wn.fixed), cy: Y(wn.b), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'window', 'data-id': wn.id, 'data-part': 'b', style: 'cursor:ns-resize' }); }
        }
      });
      ...
      geom.sliders.forEach((s) => {
        ...
        if (s.wall === 'h') {
          ...
          if (isSel) { hit += el('circle', { cx: X(s.a), cy: Y(s.fixed), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'slider', 'data-id': s.id, 'data-part': 'a', style: 'cursor:ew-resize' }); hit += el('circle', { cx: X(s.b), cy: Y(s.fixed), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'slider', 'data-id': s.id, 'data-part': 'b', style: 'cursor:ew-resize' }); }
        } else {
          ...
          if (isSel) { hit += el('circle', { cx: X(s.fixed), cy: Y(s.a), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'slider', 'data-id': s.id, 'data-part': 'a', style: 'cursor:ns-resize' }); hit += el('circle', { cx: X(s.fixed), cy: Y(s.b), r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'slider', 'data-id': s.id, 'data-part': 'b', style: 'cursor:ns-resize' }); }
        }
      });
      ...
      geom.doors.forEach((d) => {
        ...
        if (isSel) {
          ...
          hit += el('circle', { cx: ex, cy: ey, r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'door', 'data-id': d.id, 'data-part': 'w', style: 'cursor:crosshair' });
        }
      });
      ...
      geom.furniture.forEach((f) => {
        ...
        if (isSel) {
          ...
          gh += el('circle', { cx: wpx / 2, cy: hpx / 2, r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'furn', 'data-id': f.id, 'data-part': 'scale', style: 'cursor:nwse-resize' });
          // girar: arriba al centro
          gh += el('line', { x1: 0, y1: -hpx / 2, x2: 0, y2: -hpx / 2 - 18, stroke: '#c75b39', 'stroke-width': 1.4 });
          gh += el('circle', { cx: 0, cy: -hpx / 2 - 22, r: 6, fill: '#fff', stroke: '#c75b39', 'stroke-width': 2, 'data-kind': 'furn', 'data-id': f.id, 'data-part': 'rotate', style: 'cursor:grab' });
        }
    ...
    function onDown(ev) {
      ...
      drag = { kind, id, part, m0, orig: JSON.parse(JSON.stringify(obj)), blockToggle, moved: false };
      try { svgEl.setPointerCapture(ev.pointerId); } catch (e) {}
      svgEl.addEventListener('pointermove', onMove);
      svgEl.addEventListener('pointerup', onUp);
      render();
    ...
    function find(kind, id) {
      const arr = kind === 'wall' ? geom.walls : kind === 'window' ? geom.windows : kind === 'slider' ? geom.sliders : kind === 'furn' ? geom.furniture : geom.doors;
